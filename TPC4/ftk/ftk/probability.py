...
class Probability(ABC):
    ...
    def __sub__(self, other):
        ...
        return self.__class__(
            max(self.count - other.count, 0), max(self.total, other.total)
        )
    ...
    def __mul__(self, other):
        if not isinstance(other, Probability):
            return NotImplemented
        return self.__class__(
            max(self.count * other.count, 0), self.total * other.total
        )
    ...
    def __truediv__(self, other):
        ...
        return self.__class__(
            int(max(self.count / other.count, 0)), int(self.total / other.total)
        )
...
class LogarithmicProbability(Probability):
    ...
    def value(self):
        return (
            math.log(self.count / self.total)
...
def ratio(freqs, cmp_freqs):
    return {
        key: (freq / cmp_freqs.get(key, zeroProbability)) for key, freq in freqs.items()
    }
